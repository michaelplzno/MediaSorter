...
The MediaSorter suite includes several specialized tools for managing your media library:

### MediaSorter.ps1

The main scanning and classification engine. Analyzes media files across drives, extracts metadata with ffprobe, and generates CSV inventories for planning.

- Automatic classification (Movies, TV, Personal, Archives)
- Virtual season bundle detection
- Duplicate detection
- Episode parsing and show name extraction

### TheScrubber.ps1

Cleans up messy filenames by removing quality indicators, codecs, and release group tags.

- **Before:** `The.Office.S01E01.1080p.BluRay.x265-RG.mkv`
- **After:** `The Office S01E01.mkv`

See [THE_SCRUBBER.md](THE_SCRUBBER.md) for documentation.

### DriveBuilder.ps1

Builds media drives from CSV manifests generated by MediaSorter. Creates organized folder structures with optional integrity verification.

- Copies files from source to destination drive
- Extracts `.tar` season bundles automatically
- MD5 hash verification with caching (Quick/True modes)
- Resume capability for interrupted transfers
- Post-build cleanup (double folders, reclassification)

See [DRIVE_BUILDER.md](DRIVE_BUILDER.md) for documentation.

### DriveCloner.ps1

Creates verified clones of entire drives with cryptographic integrity checking.

- Complete drive-to-drive file copying
- Space verification before starting
- Generates MD5 hashes for source and destination
- Compares hashes to verify successful clone
- Resume capability (skips files with valid hashes)
- Detailed progress tracking for all phases

See [DRIVE_CLONER.md](DRIVE_CLONER.md) for documentation.

### NormalizeAudioTracks.ps1

Fixes audio track defaults by removing unwanted language tracks and setting preferred audio as default.

- Removes German/foreign audio tracks
- Sets English as default when available
- Processes entire drives recursively
- Creates backup files before modification

See [AUDIO_NORMALIZER.md](AUDIO_NORMALIZER.md) for documentation.
